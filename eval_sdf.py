...
from srt.trainer_sdf import SRTTrainer_sdf
...
if __name__ == '__main__':
    ...
    config_dict = dict(truncate_sdf=True)
    ...
    trainer = SRTTrainer_sdf(model, optimizer, cfg, device, out_dir, sdf_kwargs=cfg['training']['sdf_kwargs'], infertime=True)
    ...
    try:
        ...
        elif os.path.exists(os.path.join(out_dir, f'model_{max_it}.pt')):
    ...
    while True:
        ...
        print('Evaluating...')
        eval_dict = trainer.evaluate_infertime(val_loader)
        ...
        if args.wandb:
            ...
            if print_every > 0 and (it % print_every) == 0:
                log_str = ['{}={:.9f}'.format(k, v) for k, v in log_dict.items()]
